...
test_minimap_options_set_preset_and_threads() {
  ...
  grep -F 'minimap2 -c -x asm5 -t "${GPM_TASK_THREADS:-12}" -o result.paf' "${output_root}/runs/ds_a_vs_ref/command.sh" >/dev/null || {
...
test_alignment_engine_defaults_and_validation() {
  ...
  grep -F 'minimap2 -c -x asm10 -t "${GPM_TASK_THREADS:-10}" -o result.paf' "${default_root}/runs/ds_a_vs_ref/command.sh" >/dev/null || {
...
EOF
  ...
  grep -F 'minimap2 -x asm10 -t "${GPM_TASK_THREADS:-10}" -c --cs -o result.paf' "${output_root}/runs/chr_Chr01/cen_scan/command.sh" >/dev/null || {
