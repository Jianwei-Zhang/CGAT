...
test("bindings dispatch Subview clear and history actions", () => {
  ...
  const targets = actions.map((action) => ({
    ...
    addEventListener(type, handler) {
      listeners.set(action, { type, handler });
    },
  }));
  const host = {
    querySelector() {
    ...
    querySelectorAll(selector) {
      return selector === "[data-subview-action]" ? targets : [];
    },
    addEventListener() {},
  };
  const calls = [];
  ...
  assert.deepEqual(calls, ["clear", "rollback", "restore-rollback", "reset"]);
});
...
test("bindings dispatch current Final Path GRT baseline restore", async () => {
  const listeners = new Map();
  const restoreTarget = {
    dataset: { finalPathRestoreGrtBaseline: "Chr01" },
    disabled: false,
    addEventListener(type, handler) {
  ...
  const host = {
    ...
    querySelectorAll(selector) {
      return selector === "[data-final-path-restore-grt-baseline]" ? [restoreTarget] : [];
    },
    addEventListener() {},
  };
  const calls = [];
  const deps = createBindingDeps({
    restoreFinalPathFromGrtBaseline(_host, _store, payload) {
      calls.push(payload);
    },
  });
...
test("composition scale controls increase the visible bp span while retaining the viewport center", async () => {
  for (const [field, selector, before, after] of [
    ...
    const listeners = new Map();
    const input = {
      value: String(after), closest: () => null, setAttribute() {},
      addEventListener: (type, handler) => listeners.set(type, handler),
    };
    const host = {
      querySelector: (query) => query === selector ? input
        : query === ".subview-track-scroll" ? { clientWidth: 600 } : null,
    ...
    let persisted = 0;
    const deps = createBindingDeps({
      persistProjectAssemblyViewStateFromStore() { persisted += 1; },
    });
