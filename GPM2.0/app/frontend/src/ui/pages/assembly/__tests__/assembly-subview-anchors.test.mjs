...
test("subview track controls render independently from main track controls", () => {
  ...
  assert.match(
    html,
    /<div class="subview-panel-title-row"[^>]*>\s*<h4>Subview<\/h4>\s*<button[^>]*data-subview-tools-toggle="1"[^>]*>[\s\S]*?<\/button>\s*<button[^>]*data-subview-action="close-clear"[^>]*>[\s\S]*?<\/button>\s*<p class="muted subview-panel-guide">Ctrl\/Cmd 选中任意两个ctg进入subview-ctg；也支持点击任意两个轨道名进入subview-track。<\/p>\s*<div class="subview-candidate-row">[\s\S]*<\/div>\s*<\/div>/,
  );
