...
test("subview track controls render independently from main track controls", () => {
  ...
  assert.match(
    html,
    /<div class="subview-panel-title-row"[^>]*>\s*<h4>局部视图<\/h4>\s*<button[^>]*data-subview-tools-toggle="1"[^>]*>[\s\S]*?<\/button>\s*<p class="muted subview-panel-guide">Ctrl\/Cmd 选择两个 ctg 进入局部视图；也可选择两个轨道进入轨道模式。<\/p>\s*<div class="subview-candidate-row">[\s\S]*<\/div>\s*<\/div>/,
  );
