...
导入或打开项目后，项目页在同一张表中依次展示参考基因组、主组装、辅助组装及与当前项目关联的派生数据集，第一列为“数据类型”。Reads 质控状态和创建时间显示在项目标题旁。表格展示序列数、总长度、N50、N90；编辑按钮打开名称与备注弹窗，“更多信息”中提供初始名称、最长序列、L50、自比对可用状态及本地位置。统计使用当前数据集的全部序列长度，不过滤短序列、不按 N 拆分，也不随组装视图中的移动、隐藏而改变；无 FASTA 包同样支持这些统计，无需额外运行 Server 质控。
