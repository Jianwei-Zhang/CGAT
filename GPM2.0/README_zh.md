...
主比对图的色带在 reference gap 处分开，并贴合 reference 片段的实际显示位置。新版 Server 的 minimap2/winnowmap 比对保存 CIGAR，客户端据此确定 gap 两侧的对应坐标；旧包或不含 CIGAR 的比对只能近似裁剪，悬停提示会注明，不能当作逐碱基对应证据。生成的新流程会校验 CIGAR，旧流程需用新版 `prepare.sh` 在新工作目录重新生成，不能靠重新打开旧项目补回缺失信息。

主图工具栏的“显示”菜单可分别开关 Tel、Cen 标记，设置随项目保存。开关只影响主图标记的可见性，不修改注释、装配、GRT 或导出序列。
