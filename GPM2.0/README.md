...
`run_all.sh` keeps this staged order, stops on program errors, and resumes only from checkpoints whose input, parameter, tool, and output hashes still match.
...
Use the packaging scripts generated by `server/prepare.sh`; both run the executable GRT contract validator before creating a zip:
